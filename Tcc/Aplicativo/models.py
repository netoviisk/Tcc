...
def generate_unique_code():
    while True:
        ...
        if not Escola.objects.filter(codigo_unico=code).exists():
            return code
...
class Usuarios(AbstractBaseUser, PermissionsMixin):
    ...
    REQUIRED_FIELDS = ["usu_home"]  # Campos obrigatórios para criação de superusuário
...
class Escola(models.Model):
    codigo_unico = models.CharField(max_length=10)
