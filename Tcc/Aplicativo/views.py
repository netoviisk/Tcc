...
def cadastro_usuario(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            usuario.set_password(form.cleaned_data['usu_senha'])  # Hash seguro da senha
            usuario.save()

            if usuario.usu_tipo == 'administrador':
                nome_escola = request.POST.get('esc_name', f"Escola de {usuario.usu_home}")
                Escola.objects.create(esc_name=nome_escola, esc_desc="Descrição Padrão", usuario=usuario)
...
def login_usuario(request):
    if request.method == "POST":
        ...
        usuario = authenticate(request, username=email, password=senha)  # Autenticação segura

        if usuario:
            login(request, usuario)
            request.session["usuario_tipo"] = usuario.usu_tipo

            return redirect("painel_" + usuario.usu_tipo)  # Redireciona dinamicamente
        else:
